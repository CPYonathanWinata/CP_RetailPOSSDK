...
namespace Microsoft.Dynamics.Retail.FiscalPrinter.Triggers
{
    ...
    public sealed class PaymentTriggers : IPaymentTrigger
    {
        ...
        {
            LSRetailPosis.ApplicationLog.Log("FiscalPrinter.PaymentTriggers", "PrePayCardAuthorization", LSRetailPosis.LogTraceLevel.Trace);
            try
            {
                if (Application.Services.Peripherals.FiscalPrinter.FiscalPrinterEnabled())
                {
                    Application.Services.Peripherals.FiscalPrinter.PrePayCardAuthorization(preTriggerResult, posTransaction, cardInfo, amount);
                }
            }
            catch (Exception x)
            {
