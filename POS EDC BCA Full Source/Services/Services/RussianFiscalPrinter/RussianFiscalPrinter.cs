...
namespace Microsoft.Dynamics.Retail.FiscalPrinter.RussianFiscalPrinter
{
    ...
    public sealed partial class RussianFiscalPrinter : IFiscalPrinter
    {
        ...
        private static IEnumerable<SaleLineItem> GetCustomerOrderLinesToPrint(CustomerOrderTransaction customerOrderTransaction)
        {
            var salesItem = (SaleLineItem)
                PosApplication.Instance.BusinessLogic.Utility.CreateSaleLineItem(
                    ApplicationSettings.Terminal.StoreCurrency,
                    PosApplication.Instance.Services.Rounding,
                    customerOrderTransaction);

            salesItem.Quantity = Math.Sign(customerOrderTransaction.AmountDue) * 1;
            salesItem.Price = Math.Abs(customerOrderTransaction.AmountDue);
            salesItem.TaxGroupId = ApplicationSettings.Terminal.CustOrderTaxItemGroup;
            salesItem.SalesTaxGroupId = ApplicationSettings.Terminal.CustOrderTaxGroup;

            if (!string.IsNullOrWhiteSpace(salesItem.TaxGroupId) && !string.IsNullOrWhiteSpace(salesItem.SalesTaxGroupId))
            {
                PosApplication.Instance.Services.Tax.CalculateTax(salesItem, customerOrderTransaction);
            }

            return new SaleLineItem[] {salesItem};
        }

        private static IEnumerable<SaleLineItem> GetSaleLinesToPrint(RetailTransaction retailTransaction)
        {
            var customerOrderTransaction = retailTransaction as CustomerOrderTransaction;

            if (customerOrderTransaction != null)
            {
                return GetCustomerOrderLinesToPrint(customerOrderTransaction);
            }

            return retailTransaction.SaleItems;
        }

        private static void PostPrintCustomerOrder(CustomerOrderTransaction customerOrderTransaction, IEnumerable<SaleLineItem> printedLines)
        {
            var custOrder = customerOrderTransaction as ICustomerOrderTransactionV4;
            if (custOrder != null)
            {
                custOrder.PrepaymentTaxLines.Clear();
                //Take only first line, because customer order deposit has only one prepayment line 
                var prepaymentLine = printedLines.FirstOrDefault();
                if (prepaymentLine != null)
                {
                    foreach(var taxLine in prepaymentLine.TaxLines)
                    {
                        custOrder.PrepaymentTaxLines.Add(taxLine);
                    }
                }
            }
        }

        private static void PostPrintTransaction(RetailTransaction retailTransaction, IEnumerable<SaleLineItem> printedLines)
        {
            var customerOrderTransaction = retailTransaction as CustomerOrderTransaction;
            if (customerOrderTransaction != null)
            {
                PostPrintCustomerOrder(customerOrderTransaction, printedLines);
            }
        }

        private static bool FiscalDocumentNeeded(RetailTransaction retailTransaction)
        {
            var customerOrder = retailTransaction as CustomerOrderTransaction;
            bool result = true;

            if (customerOrder != null)
            {
                result = customerOrder.OrderType == LSRetailPosis.Transaction.CustomerOrderType.SalesOrder &&
                    retailTransaction.AmountDue != 0;
            }
        ...
        private static void PrintFiscalDocument(RetailTransaction retailTransaction)
        {
            try
            {
                if (retailTransaction is ICustomerOrderTransaction)
                {
                    throw new NotSupportedException(Resources.Translate(Resources.CustomerOrderReceiptsNotSupported));
                }
                ///////////////////////////////////////////////////

                if (!FiscalDocumentNeeded(retailTransaction))
                {
                    return;
                }

                CheckTransactionServiceConnectionIfNeeded(retailTransaction);

                BeginFiscalDocument(retailTransaction);
                
                var linesToPrint = GetSaleLinesToPrint(retailTransaction);
                ...
                PostPrintTransaction(retailTransaction, linesToPrint);
            }
        ...
        private static void CheckTransactionServiceConnectionIfNeeded(RetailTransaction retailTransaction)
        {
            if (retailTransaction is ICustomerOrderTransaction)
            {
                bool result;
                try
                {
                    result = PosApplication.Instance.TransactionServices.CheckConnection();
                }
                catch (Exception ex)
                {
                    LSRetailPosis.ApplicationExceptionHandler.HandleException("Microsoft.Dynamics.Retail.FiscalPrinter.RussianFiscalPrinter.RussianFiscalPrinter", ex);
                    result = false;
                }

                if (!result)
                {
                    ExceptionHelper.ThrowException(13010); //Could not connect to the Transaction Service.
                }
            }
        }
