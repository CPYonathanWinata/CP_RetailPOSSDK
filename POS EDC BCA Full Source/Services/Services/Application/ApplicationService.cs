...
using Microsoft.Dynamics.AX.Efd.Entities;
    ...
    public class ApplicationService : IApplicationService
    {
        ...
        static private ReceiptTransactionType GetReceiptTransType(IPosTransaction posTransaction)
        {
            ...
            else if ((retailTrans = posTransaction as IRetailTransaction) != null)
            {
                ...
                {
                    if (retailTrans.Training)
                    {
                        return ReceiptTransactionType.TrainingSale;
                    }
                    else
                    {
        ...
        static private NumberSequenceSeedType GetSeedType(ReceiptTransactionType transType)
        {
            switch (transType)
            {
                ...
                case ReceiptTransactionType.TrainingSale:
                    return NumberSequenceSeedType.ReceiptTrainingSale;
        ...
        public string GetNextReceiptId(IPosTransaction transaction)
        {
            try
            {
                ...
                {
                    // Do not consume a Receipt Id for Voids.
                    return string.Empty;
                }
                ...
                if (Functions.CountryRegion == SupportedCountryRegion.BR)
                {
                    if (retailTransaction != null && retailTransaction.SubType == RetailTransactionSubType.ConsumerEFDocument)
                    {
                        ...
                        var fiscalData = retailTransaction.FiscalData as IDictionary<string, FiscalDocument>;

                        if (fiscalData != null)
                        {
                            fiscalDocument = fiscalData.Values.LastOrDefault();
                        }

                        var receiptIdformat = "{0}-{1}";
                        ...
                        if (fiscalDocument != null)
                        {
                            ...
                            return string.Format(CultureInfo.InvariantCulture, receiptIdformat, fiscalDocument.FiscalDocumentSeries.PadLeft(3, '0'), fiscalDocument.FiscalDocumentNumber.PadLeft(9, '0'));
                        }
                    }
                }
                ...
                if (Application.Services.FiscalRegister.FiscalRegisterEnabled())
                {
                    if (Application.Services.FiscalRegister.HasReceiptId())
                        return Application.Services.FiscalRegister.GetNextReceiptId(transaction); 
                }
