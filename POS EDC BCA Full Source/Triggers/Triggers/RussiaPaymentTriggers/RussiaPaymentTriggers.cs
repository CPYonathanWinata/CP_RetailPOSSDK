...
namespace Microsoft.Dynamics.Retail.Localization.Russia.PaymentTriggers
{
    ...
    public class PaymentTriggers : IPaymentTrigger, IGlobalization
    {
        ...
        /// Validates that <paramref name="slipNumber"/> is not used in local retsil store database.
        ...
        { 
            return (from tl in retailTransaction.TenderLines
                    where !tl.Voided && string.Equals(tl.CashDocId_RU, slipNumber, StringComparison.OrdinalIgnoreCase)
                    select tl).Any();
        }
