...
namespace Microsoft.Dynamics.Retail.Pos.Interaction.WinFormsTouch
{
    ...
    { 
        ...
        private const int MaxColumnsBeforeScrolling = 8;
        ...
        private void runReport_Click(object sender, EventArgs e)
        {
            ...
            // The grid can show as many as 8 columns without scroll bar, the column's width will be autosized
            // When there are more than 8 columns, the column's width will not be autosized, and scroll bar will appear at the bottom of grid
            if (this.gridView1.Columns.Count <= MaxColumnsBeforeScrolling)
            {
        ...
        private void CreateReportCharts(DataTable outputTable, string chartCaptionText, Collection<POSReportChart> charts)
        {
            ...
            if (outputTable != null && outputTable.Rows.Count > 0 && charts != null)
            {
