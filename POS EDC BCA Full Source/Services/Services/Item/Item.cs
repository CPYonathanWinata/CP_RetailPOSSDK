    ...
    public class Item : Services.IItem
    {
       ...
       public IApplication Application { get; set; }
        ...
        public string GetProductNameByItemId(string itemID)
        {
            ...
            int catalogRecId = 0;
            string attributeRecIdRangeValue = "*";
        ...
        public void UpdateSerialNumberInfo(ISaleLineItem saleLineItem)
        {
            ...
            try
            {
                ...
                {
                    // If SalesProcessActivated flag, prompt for all non-customer order items and customer order items being picked up.
                    CustomerOrderTransaction cot = saleLineItem.RetailTransaction as CustomerOrderTransaction;
                    if ((cot == null)  //non-customer order
                        || ((cot.OrderType == LSRetailPosis.Transaction.CustomerOrderType.SalesOrder) // customer order being picked up
                            && (cot.Mode == LSRetailPosis.Transaction.CustomerOrderMode.Pickup)))
                    {
                    ...
                    using (SqlCommand command = new SqlCommand(queryString, connection))
                    {
                        ...
                        using (SqlDataReader reader = command.ExecuteReader(CommandBehavior.SingleResult))
                        {
                            ...
                            if (reader.HasRows)
                            {
                                ...
                                if (!String.IsNullOrEmpty(Utility.ToString(reader["DESCRIPTION"])))
                                {
                                    saleLineItem.Description = Utility.ToString(reader["DESCRIPTION"]);
                                }

                                if (String.IsNullOrEmpty(saleLineItem.Dimension.ColorName))
                                {
                                    saleLineItem.Dimension.ColorName = saleLineItem.Dimension.ColorId;
                                }

                                if (String.IsNullOrEmpty(saleLineItem.Dimension.SizeName))
                                {
                                    saleLineItem.Dimension.SizeName = saleLineItem.Dimension.SizeId;
                                }

                                if (String.IsNullOrEmpty(saleLineItem.Dimension.StyleName))
                                {
                                    saleLineItem.Dimension.StyleName = saleLineItem.Dimension.StyleId;
                                }

                                if (String.IsNullOrEmpty(saleLineItem.Dimension.ConfigName))
                                {
                                    saleLineItem.Dimension.ConfigName = saleLineItem.Dimension.ConfigId;
                                }
