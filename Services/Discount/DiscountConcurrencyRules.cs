...
namespace Microsoft.Dynamics.Retail.Pos.DiscountService
{
    ...
    {
        [DebuggerDisplay("OfferId = {OfferId}; DiscountGroup = {DiscountGroup}")]
        private struct OfferGroup
        {
            ...
            {
                // Check if we are comparing the same type
                if (!(obj is OfferGroup))
                    return false;

                // Call type safe version
                return this.Equals((OfferGroup)obj);
            }

            public bool Equals(OfferGroup obj)
            {
                // Compare OfferId field
                if (!string.Equals(this.OfferId, obj.OfferId, StringComparison.Ordinal))
                    return false;

                // Compare DiscountGroup field
                if (!string.Equals(this.DiscountGroup, obj.DiscountGroup, StringComparison.Ordinal))
                    return false;
            ...
            {
                return OfferId.GetHashCode() ^ DiscountGroup.GetHashCode();
            }
        }
        ...
        /// </summary>
        [DebuggerDisplay("Concurrency = {ConcurrencyMode}; Compoundable = {IsCompoundable}")]
        ...
        {
            foreach (var salesItem in retailTransaction.CalculableSalesLines)
            {
                ...
                {
                    var compoundableDiscounts = salesItem.PeriodicDiscountPossibilities.Where(d => d.IsCompoundable && d.ConcurrencyMode == ConcurrencyMode.Compounded);
                    var nonThresholdDiscounts = compoundableDiscounts.Where(d => d.PeriodicDiscountType != PeriodicDiscountItem.PeriodicDiscOfferType.Threshold);

                    foreach (PeriodicDiscountItem discount in nonThresholdDiscounts)
                    {
            ...
            if (concurrencyMode == ConcurrencyMode.Compounded)
            {
                ApplyCompoundThresholdDiscounts(retailTransaction, appliedOffers, rejectedOffers);
            }
        }

        private static void ApplyCompoundThresholdDiscounts(RetailTransaction transaction, HashSet<OfferGroup> appliedOffers, HashSet<OfferGroup> rejectedOffers)
        {
            // we can only apply a particular threshold offer once, so track which offer group is used when we do.
            var usedThresholdOffers = new Dictionary<string, OfferGroup>();

            foreach (var salesItem in transaction.CalculableSalesLines)
            {
                if (salesItem.PeriodicDiscountPossibilities.Count == 0)
                {
                    continue;
                ...
                var compoundableDiscounts = salesItem.PeriodicDiscountPossibilities.Where(d => d.IsCompoundable && d.ConcurrencyMode == ConcurrencyMode.Compounded);
                var thresholdDiscounts = compoundableDiscounts.Where(d => d.PeriodicDiscountType == PeriodicDiscountItem.PeriodicDiscOfferType.Threshold);

                foreach (PeriodicDiscountItem discount in thresholdDiscounts)
                {
                    OfferGroup offerGroup = GetOfferGroup(discount);
                    if (!rejectedOffers.Contains(offerGroup) && !appliedOffers.Contains(offerGroup) && !salesItem.DiscountLines.Contains(discount))
                    {
                        // don't apply different offer group for already-applied threshold discount
                        if (usedThresholdOffers.ContainsKey(offerGroup.OfferId) && !offerGroup.Equals(usedThresholdOffers[discount.OfferId]))
                        {
                            continue;
                        }

                        var amountForGroup = GetTotalAmountForOfferGroup(transaction, offerGroup);
                        if (amountForGroup >= discount.ThresholdAmountRequired)
                        {
                            salesItem.DiscountLines.AddLast(discount);
                            wasDiscountApplied = true;

                            usedThresholdOffers.Add(offerGroup.OfferId, offerGroup);

                            NetTracer.Information(
                                "DiscountConcurrencyRules.ApplyPeriodicDiscounts(): Applying periodic discount '{0}' to item '{1}' line '{2}'.",
                                discount.OfferId,
                                salesItem.ItemId,
                                salesItem.LineId);
                        }
        ...
        private static decimal GetTotalAmountForOfferGroup(RetailTransaction transaction, OfferGroup offerGroup)
        {
            decimal amountForOfferGroup = 0M;

            foreach (var salesLine in transaction.CalculableSalesLines)
            {
                // is line part of this offer group
                var discountLine = salesLine.PeriodicDiscountPossibilities.FirstOrDefault(l => offerGroup.Equals(GetOfferGroup(l)));
                if (discountLine != null)
                {
                    var totalDiscount = 0M;
                    foreach (var appliedDiscount in salesLine.PeriodicDiscountLines)
                    {
                        totalDiscount += appliedDiscount.GetTotalDiscountAmountForSaleItem(salesLine.Price, salesLine.Quantity);
                    }

                    amountForOfferGroup += (salesLine.Price * salesLine.Quantity) - totalDiscount;
                }
            }

            return amountForOfferGroup;
        }
        ...
        private static Dictionary<OfferGroup, Decimal> SavingsPerOfferGroup(RetailTransaction retailTransaction)
        {
            ...
            foreach (var saleItem in retailTransaction.CalculableSalesLines)
            {
