...
namespace Microsoft.Dynamics.Retail.Pos.Printing
{
    ...
    public class Printing : IPrinting
    {
        ...
        public bool PrintReceipt(FormType formType, IPosTransaction posTransaction, bool copyReceipt)
        {
            ...
            //mod by Yonathan to prevent print SO Receipt when pay Cust Order before packing slip
            if (formType == FormType.SalesOrderReceipt)// && isPackingSlipPrint == true)
            {
                result = true;
                //Printing.InternalApplication.Services.Dialog.ShowMessage("Print SO Receipt ", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {    
        ...
        public bool PrintReceiptPackSlip(FormType formType, IPosTransaction posTransaction, bool copyReceipt, bool isPackingSlipPrint = false)
        {
            if (copyReceipt)
            {
                // Trigger: PrePrintReceiptCopy trigger for the printing receipt copy operation
