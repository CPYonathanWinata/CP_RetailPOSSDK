    ...
    public class TaxCodeProvider : ITaxProvider
    {
        ...
        protected virtual string TaxSelectSqlText
        {
            get
            {
                ...
                sb.AppendLine("TAXTABLE.TAXINCLUDEINTAX, ");
        ...
        /// </summary>
        /// <param name="taxableItem">The line item.</param>
        ...
        {
            var codes = this.GetTaxCodes(taxableItem);
            ...
            {
                var codeTaxAmount = code.CalculateTaxAmount(codes);

                // sum up the tax amount for the line
                lineTaxResult.TaxAmount += codeTaxAmount;
                
                // do the same for the amounts that are exempt
                if (code.Exempt)
                {
                    lineTaxResult.HasExempt = true;
                    lineTaxResult.ExemptAmount += codeTaxAmount;
                }
            }

            // Calculate 'effective' tax rate, considering tax exempts (deduct them, if any) and tax intervals (if any)
            decimal lineTotal = taxableItem.Quantity > 0
                        ? (taxableItem.NetAmountPerUnit * taxableItem.Quantity) - taxableItem.TaxAmountInclusive
                        : 0m;
            taxableItem.TaxRatePct = lineTotal > decimal.Zero
                        ? ((lineTaxResult.TaxAmount - lineTaxResult.ExemptAmount) * 100) / lineTotal
                        : 0m;
        }

        /// <summary>
        /// Gets base price for tax included scenario.
        /// </summary>
        /// <param name="taxableItem">
        /// The taxable item.
        /// </param>
        /// <param name="codes">
        /// The tax codes.
        /// </param>
        /// <returns>
        /// The <see cref="decimal"/> base price.
        /// </returns>
            ...
            ITaxableItem taxableItem, ReadOnlyCollection<TaxCode> codes, bool collectLimits)
        {
            ...
            while (startAmount < endAmount)
            {
                ...
                if (taxLimitMax == decimal.Zero)
                {
                    ...
                    // 3d. Now remove any rates that exceed their LimitMax
                    foreach (TaxCode code in codes)
                    {
