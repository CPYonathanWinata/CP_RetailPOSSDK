...
namespace Microsoft.Dynamics.Retail.Pos.Customer
{
    ...
    public class Customer : ICustomer
    {
        ...
        public void Search(DE.IPosTransaction posTransaction)
        {
            ...
            string cardMember = string.Empty;
            DE.ICustomer customer;
            bool isUpdateIdentifier = false;

            if (GME_Var.isCustomerTypeSearch)
            {
                // Show the search dialog
                if (!GME_Var.isEmployeePurch)
                {
                    if (GME_Var.isEnrollGuest)
                    {
                        if (integration.updateIdentifier(GME_Var.lookupCardNumber, 10) == "Success")
                        {
                            isUpdateIdentifier = true;
                        }

                        if (!isUpdateIdentifier)
                        {
                ...
                {
                    customer = GME_Var.customerData;                    
                }

                if (customer != null)
                {
                    if (GME_Var.identifierCode == null || GME_Var.identifierCode == "9999999999999999999")
                        cardMember = data.checkSearchCustomerIsMember(customer.CustomerId, Connection.applicationLoc.Settings.Database.Connection.ConnectionString);
                    else
                        cardMember = data.checkSearchCustomerIsMember(customer.CustomerId, customer.PartyId.ToString(), Connection.applicationLoc.Settings.Database.Connection.ConnectionString);

                    if (GME_Var.isCardReplacementSuccess)
                    {
                        cardMember = GME_Var.cardReplacementNumber;
                    }
                    ...
                    if (cardMember != string.Empty)
                    {
                        ...
                        {
                            //integration.requestReward0302(cardMember, posTransaction.StoreId);
                            GME_Var.isSearchCustomer = true;
                            GME_Var.identifierCode = cardMember;
                        }
                        else
                        {
                            //BlankOperations.BlankOperations.ShowMsgBox("Data tidak ditemukan/service ke engage terputus");
                            GME_Var.identifierCode = cardMember;
            ...
            {
                Jacksonsoft.WaitWindow.Show(DoProgressSearch, "Please Wait ...");
            }            

            void DoProgressSearch(object sender, Jacksonsoft.WaitWindowEventArgs e)
            {
                // Show the search dialog
                if (!GME_Var.isEmployeePurch)
                {
        ...
        public DE.ICustomer Search()
        {
            ...
            {
                searchDialog.searchTerm = GME_Custom.GME_Propesties.GME_Var.searchTerm;
                //this.Application.ApplicationFramework.POSShowForm(searchDialog);
                GME_Custom.GME_Propesties.Connection.applicationLoc.ApplicationFramework.POSShowForm(searchDialog);
        ...
        public DE.ICustomer AddNew()
        {
            ...
            //if (!GME_Custom.GME_Propesties.GME_Var.isCustomerType)
            //{
            //    //CustomerData custData = new CustomerData(Application.Settings.Database.Connection, Application.Settings.Database.DataAreaID);
            //    CustomerData custData = new CustomerData(GME_Custom.GME_Propesties.Connection.applicationLoc.Settings.Database.Connection,
            //        GME_Custom.GME_Propesties.Connection.applicationLoc.Settings.Database.DataAreaID);
            ...
            //        GME_Custom.GME_Propesties.Connection.applicationLoc.ApplicationFramework.POSShowForm(newCustDialog);
