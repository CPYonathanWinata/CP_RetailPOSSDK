    ...
    public class InfoCodes : IInfoCodes
    {
                    ...
                    if (infoCodeNeeded && (infoCode.InfocodeId != null))
                    {
                        ...
                        if (result)
                        {
                            ...
                            if (infoCode.Information != null && infoCode.Information.Length > 0)
                            {
                                ...
                                {
                                    var infoCodePrompt =
                                        (infoCode.GetTranslation(storeLanguageId) == null) ? 
                                            string.Empty : // There is no translatable prompt
                                            infoCode.GetTranslation(storeLanguageId).Prompt; // Get the translated prompt

                                    if (infoCode.PrintPromptOnReceipt && (infoCodePrompt.Length != 0))
                                    {   // Set the data
                                        infoComment = infoCodePrompt;
                                        infoCommentLocalizedForWorker = infoCodePrompt;
                                    }
                                    ...
                                    if (infoCode.PrintInputOnReceipt && (infoCode.Subcode != null) && (infoCode.Subcode.Length != 0))
                                    {
                                        ...
                                        {   // Append the additional data
                                            ...
                                            infoCommentLocalizedForWorker += " - " + infoCode.Subcode; // Subcode is not translated
                                        }
                                        else
                                        {   // Set the data
                                            infoComment = infoCode.Subcode;
                                    ...
                                    if (infoCode.PrintInputNameOnReceipt && (infoCode.Information.Length != 0))
                                    {
                                        ...
                                        {   // Append the additional data
                                            ...
                                            infoCommentLocalizedForWorker += " - " + infoCode.Information; // Do not try to translate the user input
                                        }
                                        else
                                        {   // Set the data
                                            infoComment = infoCode.Information;
            ...
            InfoCodeTableRefType tableRefId, string linkedInfoCodeId, InfoCodeType infoCodeType, IInfoCodeSystem infoCodeSystem, out IInfoCodeLineItem[] selectedTriggerReasonCode)
        {
            ...
            else if ((infoCode.InputType == InfoCodeInputType.SubCodeList) || (infoCode.InputType == InfoCodeInputType.SubCodeButtons))
            {
                ...
                {
                    bool showPriceColumn = !(Functions.CountryRegion == SupportedCountryRegion.BR &&
                                             (infoCode.InfocodeId.Equals(Infocodes.ContingencyReason, StringComparison.InvariantCulture) ||
                                             infoCode.InfocodeId.Equals(Infocodes.CancelReason, StringComparison.InvariantCulture)));
                    infoSubCodeDialog = new FormInfoCodeSubCodeList(showPriceColumn);          
                }
                else
                {
