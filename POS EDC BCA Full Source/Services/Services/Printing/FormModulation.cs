...
namespace Microsoft.Dynamics.Retail.Pos.Printing
{
    ...
    public class FormModulation
    {
        ...
        private static bool isReedemption = false;
        ...
        private string GetInfoFromTransaction(FormItemInfo itemInfo, IEFTInfo eftInfo, ITenderLineItem tenderItem, RetailTransaction theTransaction)
        {
            ...
            try
            {
                if (theTransaction != null)
                {
                    ...
                    int pointVisit = GME_Custom.GME_Propesties.GME_Var.PointVisit;
                    string storeManagerId = string.Empty;
                    string storeManagerName = string.Empty;
                    string tempPKPDate = string.Empty;
                    string NamaPTPKP = string.Empty;
                    string AlamatPKP = string.Empty;

                    if (GME_Custom.GME_Propesties.GME_Var.pingStatus)
                    {
                        storeManagerId = queryData.getStoreManagerOperatorId(theTransaction.StoreId, theTransaction.TerminalId, GME_Custom.GME_Propesties.Connection.applicationLoc.Settings.Database.Connection.ConnectionString);

                        storeManagerName = queryData.getStoreManagerOperatorName(storeManagerId, GME_Custom.GME_Propesties.Connection.applicationLoc.Settings.Database.Connection.ConnectionString);

                        tempPKPDate = queryData.getPKPDate(theTransaction.StoreId, GME_Custom.GME_Propesties.Connection.applicationLoc.Settings.Database.Connection.ConnectionString);

                        NamaPTPKP = queryData.getNamaPTPKP(theTransaction.StoreId, GME_Custom.GME_Propesties.Connection.applicationLoc.Settings.Database.Connection.ConnectionString);

                        AlamatPKP = queryData.getAlamatPKP(theTransaction.StoreId, GME_Custom.GME_Propesties.Connection.applicationLoc.Settings.Database.Connection.ConnectionString);


                    }
                    else
                    {
                        storeManagerId = queryData.getStoreManagerOperatorId(theTransaction.StoreId, theTransaction.TerminalId, GME_Custom.GME_Propesties.Connection.applicationLoc.Settings.Database.OfflineConnection.ConnectionString);

                        storeManagerName = queryData.getStoreManagerOperatorName(storeManagerId, GME_Custom.GME_Propesties.Connection.applicationLoc.Settings.Database.OfflineConnection.ConnectionString);

                        tempPKPDate = queryData.getPKPDate(theTransaction.StoreId, GME_Custom.GME_Propesties.Connection.applicationLoc.Settings.Database.OfflineConnection.ConnectionString);

                        NamaPTPKP = queryData.getNamaPTPKP(theTransaction.StoreId, GME_Custom.GME_Propesties.Connection.applicationLoc.Settings.Database.OfflineConnection.ConnectionString);

                        AlamatPKP = queryData.getAlamatPKP(theTransaction.StoreId, GME_Custom.GME_Propesties.Connection.applicationLoc.Settings.Database.OfflineConnection.ConnectionString);


                    }


                    string[] reprintPoint = new string[] { };
                    if (GME_Custom.GME_Propesties.GME_Var.isReprint)
                    {
                        if (GME_Custom.GME_Propesties.GME_Var.reprintValue.Count > 0)
                        {
                            reprintPoint = GME_Custom.GME_Propesties.GME_Var.reprintValue["TBS_POINT"].Split(';');
                        }
                    }
                    ...
                    switch (itemInfo.Variable.ToUpperInvariant().Replace(" ", string.Empty))
                    {
                        ...
                        case "TRANSACTIONNUMBER":
                            return theTransaction.TransactionId ?? string.Empty;
                        case "RECEIPTNUMBER":
                            //  return theTransaction.ReceiptId; //ganti disini 
                            string receiptID = string.Empty;
                            if (!GME_Custom.GME_Propesties.GME_Var.isReprint)
                            {
                                receiptID = GME_Custom.GME_Propesties.GME_Var.receiptID;
                            }
                            else
                            {
                                if (GME_Custom.GME_Propesties.GME_Var.reprintValue.Count > 0)
                                {
                                    receiptID = GME_Custom.GME_Propesties.GME_Var.reprintValue["TBS_ReceiptId"] + "  - RE-PRINT";
                                }
                            }
                            return receiptID;
                        case "STAFF_ID":
                        ...
                        case "TOTALWITHTAX":
                            string totalwithtax = string.Empty;
                            if (!GME_Custom.GME_Propesties.GME_Var.isReprint)
                            {
                                if (!GME_Custom.GME_Propesties.GME_Var.isReedem)
                                {
                                    totalwithtax = Printing.InternalApplication.Services.Rounding.Round(theTransaction.NetAmountWithTaxAndCharges, theTransaction.StoreCurrencyCode, true);
                                }
                                else
                                {
                                    totalwithtax = "0";
                        ...
                        case "TOTAL":
                            ...
                            if (!GME_Custom.GME_Propesties.GME_Var.isReprint)
                            {
                                if (!GME_Custom.GME_Propesties.GME_Var.isReedem)
                                {
                                    dpp = Printing.InternalApplication.Services.Rounding.Round(GME_Custom.GME_Propesties.GME_Var.totalDPP, theTransaction.StoreCurrencyCode, true);
                                }
                                else
                                {
                                    dpp = "0";
                            ...
                            else
                            {
                                ...
                                if (!isReedemption)
                                {
                                    if (GME_Custom.GME_Propesties.GME_Var.reprintValue.Count > 0)
                                    {
                                        dpp = Printing.InternalApplication.Services.Rounding.Round(Convert.ToDecimal(GME_Custom.GME_Propesties.GME_Var.reprintValue["TBS_DPP"]), theTransaction.StoreCurrencyCode, true);
                                    }
                                }
                                else
                                {
                        ...
                        case "TAXTOTAL":
                            ...
                            if (!GME_Custom.GME_Propesties.GME_Var.isReprint)
                            {
                                if (!GME_Custom.GME_Propesties.GME_Var.isReedem)
                                {
                                    taxtotal = Printing.InternalApplication.Services.Rounding.Round(theTransaction.TaxAmount, theTransaction.StoreCurrencyCode, true);
                                }
                                else
                                {
                                    taxtotal = "0";
                        ...
                        case "CUSTOMERNAME":
                            return theTransaction.Customer.Name ?? string.Empty;
                        ...
                        case "RECEIPTNUMBERBARCODE": //ganti disini
                            return "<B: " + theTransaction.ReceiptId + ">";
                        // return "<B: " + GME_Custom.GME_Propesties.GME_Var.receiptID + ">";
                        ...
                        case "STORENAME":
                            return GME_Custom.GME_Propesties.GME_Var.customerPosTransaction.StoreName;
                        case "STOREADDRESS":
                        ...
                        case "POINT":
                            ...
                            if (!GME_Custom.GME_Propesties.GME_Var.isReprint)
                            {
                                if (GME_Custom.GME_Propesties.GME_Var.lookupCardStatus != "VALID" && GME_Custom.GME_Propesties.GME_Var.identifierCode != "9999999999999999999" && !GME_Custom.GME_Propesties.GME_Var.isUpdatePointAX)
                                {
                                    point = "-";
                                }
                                else
                                {
                                    point = Convert.ToInt32(GME_Custom.GME_Propesties.GME_Var.memberPoint.ToString()).ToString();
                                }
                            }
                            else
                            {
                                if (GME_Custom.GME_Propesties.GME_Var.reprintValue.Count > 0)
                                {
                                    if (reprintPoint.Length > 0)
                                    {
                                        point = reprintPoint[0];
                                    }
                        ...
                        case "POINTVISIT":
                            ...
                            else
                            {
                                if (GME_Custom.GME_Propesties.GME_Var.reprintValue.Count > 0)
                                {
                                    if (reprintPoint.Length > 1)
                                    {
                                        pointV = reprintPoint[1];
                                    }
                        ...
                        case "REEDEMPOINT":
                            ...
                            if (!GME_Custom.GME_Propesties.GME_Var.isReprint)
                            {
                                ...
                                {
                                    //negate the value
                                    if(GME_Custom.GME_Propesties.GME_Var.pointUsed > 0)
                                    {
                                        int tempReedem = GME_Custom.GME_Propesties.GME_Var.pointUsed * -1;
                                        reedemPoint = tempReedem.ToString();
                                    }
                                    else
                                    {
                                        reedemPoint = GME_Custom.GME_Propesties.GME_Var.pointUsed.ToString();
                                    }
                            ...
                            else
                            {
                                if (GME_Custom.GME_Propesties.GME_Var.reprintValue.Count > 0)
                                {
                                    if (reprintPoint.Length > 3)
                                    {
                                        reedemPoint = reprintPoint[3];
                                    }
                        ...
                        case "TOTALPOINT":
                            ...
                            if (!GME_Custom.GME_Propesties.GME_Var.isReprint)
                            {
                                ...
                                else
                                {
                                    ...
                                    totalPoint = GME_Custom.GME_Propesties.GME_Var.totalPoint.ToString();
                                }
                            }
                            else
                            {
                                if (GME_Custom.GME_Propesties.GME_Var.reprintValue.Count > 0)
                                {
                                    if (reprintPoint.Length > 2)
                                    {
                                        totalPoint = reprintPoint[2];
                                    }
                        ...
                        case "APPROVALCODE":
                            ...
                            if (!GME_Custom.GME_Propesties.GME_Var.isReprint)
                            {
                                if (GME_Custom.GME_Propesties.GME_Var.approvalCodeBCA != string.Empty)
                                {
                                    approvalCode = GME_Custom.GME_Propesties.GME_Var.approvalCodeBCA;
                                }
                                else if (GME_Custom.GME_Propesties.GME_Var.approvalCodeMandiri != string.Empty)
                                {
                                    approvalCode = GME_Custom.GME_Propesties.GME_Var.approvalCodeMandiri;
                                }
                                else if (GME_Custom.GME_Propesties.GME_Var.payCardApprovalCodeOffline != string.Empty)
                                {
                                    approvalCode = GME_Custom.GME_Propesties.GME_Var.payCardApprovalCodeOffline;
                                }
                            }
                            else
                            {
                                if (GME_Custom.GME_Propesties.GME_Var.reprintValue.Count > 0)
                                {
                                    approvalCode = GME_Custom.GME_Propesties.GME_Var.reprintValue["TBS_ApprovalCode"];
                                }
                            }
                            return approvalCode;
                        case "STOREMANAGERID":
                        ...
                        case "DATEPKP":
                            if (tempPKPDate != null)
                            {
                                DateTime PKPDate = Convert.ToDateTime(tempPKPDate);
                                return String.Format("{0:dd - MMM - yyyy}", PKPDate);
                            }
                            else
                            {
                                return string.Empty;
                        ...
                        case "PANNUMBER":
                            ...
                            if (!GME_Custom.GME_Propesties.GME_Var.isReprint)
                            {
                                if (GME_Custom.GME_Propesties.GME_Var.panBCA != string.Empty && GME_Custom.GME_Propesties.GME_Var.panBCA.Length > 0)
                                {
                                    PanNumber = "**" + GME_Custom.GME_Propesties.GME_Var.panBCA.Substring(6);
                                }
                                else if (GME_Custom.GME_Propesties.GME_Var.panMandiri != string.Empty && GME_Custom.GME_Propesties.GME_Var.panMandiri.Length > 0)
                                {
                                    PanNumber = "**" + GME_Custom.GME_Propesties.GME_Var.panMandiri.Substring(6);
                                }
                            }
                            else
                            {
                                if (GME_Custom.GME_Propesties.GME_Var.reprintValue.Count > 0)
                                {
                                    PanNumber = GME_Custom.GME_Propesties.GME_Var.reprintValue["TBS_PAN"];
                                }
                        ...
                        case "PTPKPNAME":
                            return NamaPTPKP;
                        case "ALAMATPKP":
                            AlamatPKP = AlamatPKP.Replace("\\r\\n", Environment.NewLine);
                            return AlamatPKP;
                        case "POINTMESSAGE":
                            string pointmessage = string.Empty;
                            if (!GME_Custom.GME_Propesties.GME_Var.isReprint)
                            {
                                if (GME_Custom.GME_Propesties.GME_Var.lookupCardStatus != "VALID" && GME_Custom.GME_Propesties.GME_Var.identifierCode != "9999999999999999999" && !GME_Custom.GME_Propesties.GME_Var.isUpdatePointAX)
                                {
                                    pointmessage = "Point & Voucher akan update dalam waktu 1 x 24 jam ";
                                }
                                else
                                {
                                    pointmessage = string.Empty;
                                }
                            }
                            return pointmessage;
            ...
            try
            {
                ...
                switch (itemInfo.Variable.ToUpperInvariant().Replace(" ", string.Empty))
                {
                    ...
                    case "UNITPRICE":
                        ...
                        {
                            returnValue = Printing.InternalApplication.Services.Rounding.Round(decimal.Negate(saleLine.Price), false);
                        }
                        else
                        {
                    ...
                    case "TOTALPRICEWITHTAX":
                        if (saleLine.Quantity < 0)
                        {
                            returnValue = Printing.InternalApplication.Services.Rounding.Round(decimal.Negate(saleLine.NetAmountWithTax), false);
                        }
                        else
                        {
                            returnValue = Printing.InternalApplication.Services.Rounding.Round(Math.Abs(saleLine.NetAmountWithTax), false);
                        }                       
                        break;
                    case "ITEMUNITID":
                    ...
                    case "TOTALDISCOUNTAMOUNT":
                        ...
                        if (!GME_Custom.GME_Propesties.GME_Var.isReprint)
                        {
                            if (GME_Custom.GME_Propesties.GME_Var.isReedem)
                            {
                                totalDisc = saleLine.NetAmount;
                            }
                            else
                            {
                                foreach (LSRetailPosis.Transaction.Line.Discount.DiscountItem disc in saleLine.DiscountLines)
                                {
                                    totalDisc += disc.EffectiveAmount;
                                }
                            }
                        ...
                        //if (saleLine.TotalDiscount != 0)
                        //{
                        //    totalDisc += Math.Abs(saleLine.TotalDiscount); 
                        //   // returnValue = Printing.InternalApplication.Services.Rounding.Round(decimal.Negate(saleLine.TotalDiscount), false);
                        //} else if (saleLine.PeriodicDiscount != 0){
                        //    totalDisc += Math.Abs(saleLine.PeriodicDiscount);
                        //    //returnValue = Printing.InternalApplication.Services.Rounding.Round(decimal.Negate(saleLine.PeriodicDiscount), false);
                        //}
                        //else if (saleLine.LineDiscount != 0)
                        //{
                        //    totalDisc += Math.Abs(saleLine.LineDiscount);
                        //    //returnValue = Printing.InternalApplication.Services.Rounding.Round(decimal.Negate(saleLine.LineDiscount), false);
                        //}

                        returnValue = Printing.InternalApplication.Services.Rounding.Round(decimal.Negate(totalDisc), false);
                    ...
                    switch (itemInfo.Variable.ToUpperInvariant().Replace(" ", string.Empty))
                    {
                        case "TENDERNAME":
                            if (tenderLine.Amount < 0)
                            {
                                ...
                                else if (tenderLine.TenderTypeId == "17")
                                {
                                    returnValue += " ( EDC MANDIRI )";
                                }
                                else
                                {
                            ...
                            break;
                        case "TENDERAMOUNT":
                            if (isReedemption || GME_Custom.GME_Propesties.GME_Var.isReedem)
                            {
                                returnValue = "0";
                            }
                            else
        ...
        private string ParseVariable(FormItemInfo itemInfo, ITenderLineItem tenderItem, RetailTransaction theTransaction)
        {
            ...
            if (tmpString.IndexOf("\n") != -1)
            {
                ...
                for (int i = 0; i <= tmpNewLines.Length - 1; i++)
                {
                    ...
                    if (i == 0)
                    {
                        if (itemInfo.Variable == "AlamatPKP")
                        {
                            tmpString = tmpNewLines[i].TrimEnd();
                        }
                        else
                        {
        ...
        private string ReadDataset(DataSet ds, ITenderLineItem tenderItem, RetailTransaction theTransaction)
        {
            ...
            if (GME_Custom.GME_Propesties.GME_Var.isReprint)
            {
                System.Collections.Generic.LinkedList<TenderLineItem> paymentMethodsLines;
                if (theTransaction is SalesInvoiceTransaction)
                {
                    paymentMethodsLines = ((SalesInvoiceTransaction)theTransaction).TenderLines;
                }
                else if (theTransaction is SalesOrderTransaction)
                {
                    paymentMethodsLines = ((SalesOrderTransaction)theTransaction).TenderLines;
                }
                else
                {
                    paymentMethodsLines = theTransaction.TenderLines;
                }

                foreach (ITenderLineItem tenderLineItem in paymentMethodsLines)
                {
                    if (!tenderLineItem.Voided)
                    {
                        if (tenderLineItem.TenderTypeId == "3")
                        {
                            isReedemption = true;
                        }
                        else
