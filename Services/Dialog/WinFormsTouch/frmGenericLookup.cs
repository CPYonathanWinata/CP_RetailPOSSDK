...
namespace Microsoft.Dynamics.Retail.Pos.Dialog.WinFormsTouch
{
	...
	public class frmGenericLookup : frmTouchBase
    {
        ...
        /// </summary>
        /// <param name="dataTable">List of classes with properties</param>
        /// <param name="displayProperties">Display propertites which should be displayed</param>
        /// <param name="displayHeading">Display heading</param>
        /// <param name="returnPropertyName">Name of property whose value is saved in SelectedString property</param>
        /// <param name="defaultValue">Default focused instance when form opens</param>       
        public frmGenericLookup(IList dataTable, OrderedDictionary displayProperties, string displayHeading, string returnPropertyName, string defaultValue)
            : this()
        {
            if (dataTable == null)
            {
            ...
            {
                throw new ArgumentNullException("displayProperties");
            }

            labelHeading.Text = displayHeading;
            var elementType = dataTable.AsQueryable().ElementType;          
            this.returnProperty = elementType.GetProperty(returnPropertyName) ?? elementType.GetProperties()[0];
            ...
            foreach (var property in elementType.GetProperties())
            {
                AddGridColumn(displayProperties.Contains(property.Name),
                                property.PropertyType == typeof(String),
                                displayProperties.Contains(property.Name) ? displayProperties[property.Name].ToString() : property.Name,
                                property.Name);

                displayColumnNum = displayProperties.Contains(property.Name) ? index : displayColumnNum;
                returnColumnNum = (property == returnProperty) ? index : returnColumnNum;
                index += 1;
            }
