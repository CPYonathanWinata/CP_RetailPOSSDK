...
namespace Microsoft.Dynamics.Retail.Pos.Dialog.WinFormsTouch
{
    ...
    partial class frmTenderCount : frmTouchBase
    {
        ...
        private string currencyTenderTypeID = string.Empty;
        ...
        private const int STRING_ID_CASH_TENDER_NAME = 1965; // Cash - {0}
        private const int STRING_ID_CARD_TENDER_NAME = 1966; // Card - {0}
        ...
        protected override void OnLoad(EventArgs e)
        {
            if (!this.DesignMode)
            {
                ...
                currencyTenderTypeID = tenderData.GetTenderID(ApplicationSettings.Terminal.StoreId, ((int)(PosisOperations.PayCurrency)).ToString());
                    ...
                    switch (tmpTender.PosisOperation)
                    {
                        case PosisOperations.PayCash:
                            ...
                            TenderViewModel cashRow = new TenderViewModel()
                            {
                                ...
                                DisplayName = ApplicationLocalizer.Language.Translate(STRING_ID_CASH_TENDER_NAME, tmpTender.TenderName),
                        ...
                        case PosisOperations.PayCurrency:
                                                            ...
                                                            ApplicationSettings.Terminal.StorePrimaryId))
                            {
                                TenderViewModel currencyRow = new TenderViewModel()
                                {
                                    ...
                                    TenderOperationType = tmpTender.PosisOperation,
                                    TenderName = tmpTender.TenderName,
                                    DisplayName = ApplicationLocalizer.Language.Translate(STRING_ID_CASH_TENDER_NAME, currency),
                                    Currency = currency
                                };
                        ...
                        case PosisOperations.PayCard:
                            ...
                            foreach (ICardInfo card in cards)
                            {
                                TenderViewModel cardRow = new TenderViewModel()
                                {
                                    ...
                                    DisplayName = ApplicationLocalizer.Language.Translate(STRING_ID_CARD_TENDER_NAME, card.CardName),
                                };
        ...
        {
            if (!currentTenderTypeID.Equals(currencyTenderTypeID, StringComparison.OrdinalIgnoreCase))
            {
            ...
            {
                gridSource.First(tender =>
                   tender.Currency.Equals(currentTenderName, StringComparison.OrdinalIgnoreCase)).Total = 0m;
            }
        ...
        private bool ProcessTenderCountLine(List<TenderViewModel> tenderCountLines, PosisOperations tenderOperationType, ref int attemptsDoneCounter)
        {
            ...
            foreach (TenderViewModel tenderCountLine in tenderCountLines)
            {
                if (transaction is TenderDeclarationTransaction)
                {
                    ...
                    if (NeedForRecount(tenderCountLine.TenderTypeId, tenderCountLine.CardTypeId, tenderCountLineTotal, tenderCountLine.Currency, ref attemptsDoneCounter))
                    {
                        // Select the cash row
                        if (rowHandle != -1)
                        {
        ...
        {
            // Check whether there has been a value entered
            bool gridHasNonZeroValue = gridSource.Any(tender => tender.Total != 0m);

            if (gridHasNonZeroValue)
            {
        ...
        private decimal GetExpectedAmount(string testTender, string testTenderName, string testCard)
        {
            ...
            foreach (DataRow row in requiredAmounts.Rows)
            {
                ...
                if (tenderID.Equals(testTender, StringComparison.Ordinal) && (string.IsNullOrEmpty(testCard) || cardTypeID.Equals(testCard, StringComparison.Ordinal)))
                {
                    if (!tenderID.Equals(currencyTenderTypeID, StringComparison.OrdinalIgnoreCase))
                    {
