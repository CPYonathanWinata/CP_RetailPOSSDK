...
namespace Microsoft.Dynamics.Retail.Pos.Dimension
{
    ...
    public class Dimension : IDimension
    {
        ...
        const string querySelectClause = @" 
            ...
            FROM [dbo].GETASSORTEDPRODUCTS(@STORERECID, @STOREDATE) AIT 
            INNER JOIN INVENTDIMCOMBINATION IDC ON IDC.ITEMID = AIT.ITEMID AND IDC.DATAAREAID = @DATAAREAID 
	            AND (IDC.DISTINCTPRODUCTVARIANT = AIT.DISTINCTPRODUCTVARIANT OR AIT.DISTINCTPRODUCTVARIANT = 0)
        ...
        public System.Data.DataTable GetDimensions(string itemId)
        {
            ...
            try
            {
                string sql = querySelectClause + @" WHERE IDC.ITEMID = @ITEMID";
        ...
        public void GetDimensionForVariant(DE.IDimension dimension)
        {
            ...
            try
            {
                string sql = querySelectClause + @" WHERE IDC.RETAILVARIANTID = @RETAILVARIANTID";
