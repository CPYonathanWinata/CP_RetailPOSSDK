...
namespace Microsoft.Dynamics.Retail.Pos.Dialog.WinFormsTouch
{
    ...
    partial class frmTenderCount : frmTouchBase
    {
                    ...
                    switch (tmpTender.PosisOperation)
                    {
                        ...
                        case PosisOperations.PayCard:
                            ...
                            foreach (ICardInfo card in cards)
                            {
                                ...
                                if (card.CardTypeId == "EDC BCA")
                                {
                                    decimal amountBCA = 0;

                                    if (GME_Custom.GME_Propesties.GME_Var.amountBCA != string.Empty)
                                        amountBCA = Convert.ToDecimal(GME_Custom.GME_Propesties.GME_Var.amountBCA);
                                ...
                                if (card.CardTypeId == "EDC MDR")
                                {
                                    decimal amountMandiri = 0;

                                    if (GME_Custom.GME_Propesties.GME_Var.amounMandiri != string.Empty)
                                        amountMandiri = Convert.ToDecimal(GME_Custom.GME_Propesties.GME_Var.amounMandiri);
        ...
        protected override void OnClosing(CancelEventArgs e)
        {
            ...
            {
                GME_Custom.GME_Propesties.GME_Var.isTenderCounted = true;
