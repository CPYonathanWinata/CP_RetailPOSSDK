    ...
    public class Price : IPrice, IInitializeable
    {
        ...
        public void GetPrice(IRetailTransaction retailTransaction)
        {
                ...
                && transaction.CalculableSalesLines.Any())
            {
                ...
                using (CommerceRuntime commerceRuntime = CommerceRuntime.Create(ApplicationSettings.CommerceRuntimeConfiguration, principal))
                {
                    ...
                    GetPriceServiceResponse response = svc.Execute<GetPriceServiceResponse>(new GetPriceServiceRequest(requestContext, crtTransaction));
        ...
        public void UpdateAllPrices(IRetailTransaction retailTransaction, bool restoreItemPrices)
        {
            ...
            using (CommerceRuntime commerceRuntime = CommerceRuntime.Create(ApplicationSettings.CommerceRuntimeConfiguration, principal))
            {
                ...
                CRTDM.SalesTransaction crtTransaction = CommerceRuntimeTransactionConverter.ConvertRetailTransactionToSalesTransaction(transaction);
                Microsoft.Dynamics.Commerce.Runtime.Services.IService svc = commerceRuntime.GetService(ServiceTypes.PricingService);
                GetPriceServiceResponse response = svc.Execute<GetPriceServiceResponse>(new UpdatePriceServiceRequest(requestContext, crtTransaction, restoreItemPrices));
                CommerceRuntimeTransactionConverter.PopulateRetailTransactionFromSalesTransaction(response.Transaction, transaction);
            }
        }
        ...
        public IEnumerable<ISaleLineItem> GetItemPrices(IEnumerable<ISaleLineItem> items, string inventDimId)
        {
            ...
            using (CommerceRuntime commerceRuntime = CommerceRuntime.Create(ApplicationSettings.CommerceRuntimeConfiguration, principal))
            {
                ...
                GetPricesServiceRequest priceRequest = new GetPricesServiceRequest(requestContext, salesLines, requestContext.ChannelConfiguration.GetCurrentDateTimeOffsetInChannelTimeZone().Date, string.Empty, string.Empty, CRTDM.PricingCalculationMode.Independent);
                GetPricesServiceResponse response = svc.Execute<GetPricesServiceResponse>(priceRequest);
