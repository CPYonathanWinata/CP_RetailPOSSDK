...
namespace Microsoft.Dynamics.Retail.Pos.Interaction
{
    ...
    public partial class RedeemLoyaltyPointsForm : frmTouchBase, IInteractionView
    {
            ...
            : this()
        {
            ...
            padLoyaltyCardNumber.Enabled = false;
            padLoyaltyDiscountAmount.NumberOfDecimals = 0;
            padLoyaltyDiscountAmount.EntryStartsInDecimals = true;
        }
        ...
        protected override void OnClosing(System.ComponentModel.CancelEventArgs e)
        {
            if (DialogResult == System.Windows.Forms.DialogResult.OK)
            {
                ...
                decimal reedemPoint = Convert.ToDecimal(queryData.getRedeemAmountDefault(GME_Custom.GME_Propesties.Connection.applicationLoc.Settings.Database.Connection.ConnectionString));
                ...
                if (reedemPoint != null)
                {
                    if (reedemPoint != 0)
                    {
                        ...
                        if (amount % reedemPoint != 0)
                        {
                            BlankOperations.BlankOperations.ShowMsgBoxInformation("Point yang anda masukkan harus kelipatan " + Convert.ToInt32(reedemPoint).ToString());
                            e.Cancel = true;
                        }
                        else
                        {
                ...
                {
                    BlankOperations.BlankOperations.ShowMsgBoxInformation("Terdapat masalah untuk melakukan redeemption katalog");
                    e.Cancel = true;
                }
