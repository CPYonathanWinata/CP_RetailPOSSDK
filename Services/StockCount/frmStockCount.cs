...
namespace Microsoft.Dynamics.Retail.Pos.StockCount
{
    ...
    public partial class frmStockCount : LSRetailPosis.POSProcesses.frmTouchBase
    {
        ...
        private void LoadStockCountsFromDB()
        {
            try
            {
                // Get all items from stock count which are not assorted.
                ReadOnlyCollection<string> unassortedItems = scData.GetUnassortedItemsFromStockCount(this.SelectedJournalId);
                bool createdLocal;
                ...
                if (unassortedItems != null && unassortedItems.Count > 0)
                {
                    ...
                    XDocument tmpstring = StockCount.InternalApplication.Services.Item.GetProductData(itemIds);
            ...
            catch (Exception ex)
            {
                ...
                StockCount.InternalApplication.Services.Dialog.ShowMessage(99890, MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        ...
        private void InventoryLookup(string barcode)
        {
            ...
            else if ((inputMode == NumPadMode.Quantity) && (saleLineItem != null) && !string.IsNullOrEmpty(numPad1.EnteredValue))
            {
                UnitOfMeasureData unitOfMeasureData = new UnitOfMeasureData(
                    StockCount.InternalApplication.Settings.Database.Connection,
                    StockCount.InternalApplication.Settings.Database.DataAreaID,
                ...
                item.Quantity = StockCount.InternalApplication.Services.Rounding.Round(quantity, unitOfMeasureData.GetUnitDecimals(item.Unit));
        ...
        private DataTable AddVariantInfo(DataTable countTableComment)
        {
            ...
            foreach (DataRow row in countTableComment.Rows)
            {
                ...
                saleLineItem.Dimension.ColorId = row[DataAccessConstants.InventColorId] as string;
                saleLineItem.Dimension.SizeId = row[DataAccessConstants.InventSizeId] as string;
                saleLineItem.Dimension.StyleId = row[DataAccessConstants.InventStyleId] as string;
                saleLineItem.Dimension.ConfigId = row[DataAccessConstants.ConfigId] as string;
        ...
        {
            string dash = " - ";
            string color = Dimensions.DimensionValue(saleItem.Dimension.ColorName, saleItem.Dimension.ColorId);
            string size = Dimensions.DimensionValue(saleItem.Dimension.SizeName, saleItem.Dimension.SizeId);
            string style = Dimensions.DimensionValue(saleItem.Dimension.StyleName, saleItem.Dimension.StyleId);
            string config = Dimensions.DimensionValue(saleItem.Dimension.ConfigName, saleItem.Dimension.ConfigId);
