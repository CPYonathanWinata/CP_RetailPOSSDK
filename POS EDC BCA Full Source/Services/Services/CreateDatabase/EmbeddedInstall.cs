...
namespace Microsoft.Dynamics.Retail.Pos.CreateDatabaseService
{
    public class EmbeddedInstall : ICreateDatabase
    {
        ...
        /// <summary>
        /// Checks if channel database needs upgrade.
        /// </summary>
        /// <param name="connectionString">Channel database connection string.</param>
        /// <returns>If true, needs upgrade, otherwise does not need upgrade.</returns>
        public bool IsDatabaseUpgradeRequired(string connectionString)
        {
            try
            {
            ...
            {
                string message = string.Format("Determining if database requires upgrade failed:{0}", ex.ToString());
                LogMessage(message);
        ...
        public void CheckDatabase(string connectionString, string databasePhysicalFilePath, bool includeDemoData, string userGroupName)
        {
            try
            {
                ...
                // Flag indicates if database exists before running database script.
                bool doesDatabaseExist = false;
                ...
                using (progressForm = new ProgressForm())
                {
                    ...
                    using (DbConnection dBConnection = new SqlConnection(connectionString))
                    {
                        using (DbCommand dBCommand = new SqlCommand())
                        {
                            ...
                            // Always run the schema script to upgrade to the latest version.
                            try
                            {
                            ...
                            catch (Exception ex)
                            {
                                if (!doesDatabaseExist)
                                {
                                    ...
                                    dBCommand.ExecuteScalar(queryString);                                    
                                }

                                string message = string.Format("CheckDatabase failed while creating database schema:{0}", ex.ToString());
                                LogMessage(message);
