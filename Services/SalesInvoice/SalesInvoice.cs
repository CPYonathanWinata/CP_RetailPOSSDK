...
namespace Microsoft.Dynamics.Retail.Pos.SalesInvoice
{
    ...
    public class SalesInvoice : ISalesInvoice
    {
        ...
        /// <summary>
        /// Record payment of a single Sales Invoice Line in Headquarters by calling TS PaySalesInvoice API.
        /// </summary>
        /// <param name="retValue">True if successful, false otherwise.</param>
        /// <param name="comment">Optional comment or error message from headquarters.</param>
        /// <param name="invoiceLine">The sales invoice line to pay.</param>
        /// <param name="transaction">The transaction tha the sales invoice line is part of.</param>
        public void PaySalesInvoice(ref bool retValue, ref string comment, ISalesInvoiceLineItem invoiceLine, IPosTransaction transaction)
        {
            if (invoiceLine == null)
            {
                throw new ArgumentNullException("invoiceLine");
            }
            ...
            ReadOnlyCollection<object> results = this.Application.TransactionServices.Invoke(
                "PaySalesInvoice",
                 new object[] { 
            ...
            retValue = Convert.ToBoolean(results[1]);
            comment = Convert.ToString(results[2]);
        }
