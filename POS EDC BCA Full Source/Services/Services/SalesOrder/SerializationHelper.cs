...
namespace Microsoft.Dynamics.Retail.Pos.SalesOrder
{
    internal static class SerializationHelper
    {
        ...
        internal static CustomerOrderInfo GetInfoFromTransaction(CustomerOrderTransaction customerOrder)
        {
            ...
            // Payments
            CustomerOrderPrepaymentTaxAllocator paymentTaxAllocator = new CustomerOrderPrepaymentTaxAllocator(customerOrder);
                ...
                if (!tender.Voided)
                {
                    ...
                    else if (tender.Amount != decimal.Zero)
                    {
                        ...
                        var paymentInfo = new PaymentInfo()
                        {
                            ...
                            Currency = customerOrder.StoreCurrencyCode,
                            PrepaymentTaxLines = new Collection<TaxLineInfo>()
                        };
                        parameters.Payments.Add(paymentInfo);
                        paymentTaxAllocator.AddPayment(tender, paymentInfo);
                    }
                }
            }

            paymentTaxAllocator.ProcessOrderPrepayment();
